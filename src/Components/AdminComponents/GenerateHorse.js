...
export default function NewHorse() {
    ...
    const handleSubmit = () => {
        if (quantity<1) {
            ...
            setAlert(false);
            axios.post('https://localhost:7090/api/Horse/GenerateHorse', quantity)
            .then((response) => {
    ...
    return (
        <>
            <FormGroup spacing={2}>
                ...
                <TextField id="outlined-basic" label="Generation Count" variant="outlined" name="name" value={quantity} onChange={e => setQuantity(e.target.value)}/>
                {alert?
